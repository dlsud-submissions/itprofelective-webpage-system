...
- **Direct navigation to `/login` shows "Cannot GET /login"**: this is a pre-existing issue in this repo's `vite.config.ts` dev proxy (it forwards _all_ methods for `/login`, including a plain page `GET`, straight to the Express API, which only handles `POST /login`). It affects Epic A/C's login page, not anything built in this issue — always reach `/login` by clicking the **Sign In** link from the landing page (a client-side route change, which the proxy never sees) rather than typing the URL directly.
