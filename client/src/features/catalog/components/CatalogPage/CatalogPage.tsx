...
export function CatalogPage<T extends CatalogItem>({
  ...
  const handleCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const result = await createItem({
      ...
      price,
      [extraField.key]: createDraft.extra
        ? parseExtraValue(extraField, createDraft.extra)
        : undefined,
    });
    setIsCreating(false);
  ...
  const startEdit = (item: T) => {
    ...
    setEditDraft({
      ...
      extra:
        item[extraField.key] !== undefined ? String(item[extraField.key]) : '',
    });
  };
  ...
  const saveEdit = async (id: string) => {
    ...
    const result = await updateItem(id, {
      ...
      price,
      [extraField.key]: editDraft.extra
        ? parseExtraValue(extraField, editDraft.extra)
        : undefined,
    });
    ...
    setItems((current) =>
      current.map((it) => (it.id === id ? (result.data as T) : it))
    );
    cancelEdit();
  };
  ...
  const toggleActive = async (item: T) => {
    ...
    setItems((current) =>
      current.map((it) => (it.id === item.id ? (result.data as T) : it))
    );
  };
          ...
          >
            ...
            <label className={styles.field}>
              ...
              <input
                ...
                onChange={(event) =>
                  setCreateDraft((d) => ({ ...d, price: event.target.value }))
                }
              />
            </label>
            <label className={styles.field}>
              ...
              <input
                ...
                onChange={(event) =>
                  setCreateDraft((d) => ({ ...d, extra: event.target.value }))
                }
              />
            </label>
            {createError ? (
